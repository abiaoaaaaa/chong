...
class ElectricVehicleEnv(gym.Env):
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(
            ...
            high=np.array(
                [self.num_nodes - 1, self.evs_50['电池容量'].max(), self.evs_50['截止时间'].max(), self.num_nodes - 1],
                dtype=np.float32),
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        distance_reward = distance_to_target_start - distance_to_target_end  # 距离接近终点越多，奖励越大
        # 计算奖励和判断是否完成任务
        self.reward = -energy_consumed
        self.reward += distance_reward  # 距离接近目标的正奖励
        if self.roads_50.iloc[self.last_node, next_node] == 1:  # 判断是否在充电路段
            ...
            charging = True
            self.reward += energy_charged  # 充电的正奖励

        if self.current_node == self.target_node:
            self.reward += self.current_battery  # 到达终点后奖励剩余电量
            self.done = True
        elif self.remaining_time <= 0 or self.current_battery <= 0:
            self.done = True
            self.reward -= 100  # 任务失败的惩罚
